...
resource "aws_subnet" "new_private_subnet_2c" {
  vpc_id = aws_vpc.new_vpc.id
  cidr_block = "${cidr_private1}"
  map_public_ip_on_launch = true
...
resource "aws_subnet" "new_private_subnet_2d" {
  vpc_id = aws_vpc.new_vpc.id
  cidr_block = "${cidr_private2}"
  map_public_ip_on_launch = true
...
resource "aws_route_table_association" "new_private_subnet_2c_association" {
  subnet_id = aws_subnet.new_private_subnet_2c.id
  route_table_id = aws_route_table.new_public_rtb.id
}
resource "aws_route_table_association" "new_private_subnet_2d_association" {
  subnet_id = aws_subnet.new_private_subnet_2d.id
  route_table_id = aws_route_table.new_public_rtb.id
}
